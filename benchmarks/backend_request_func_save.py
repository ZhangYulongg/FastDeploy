...
def messages_to_history_qa(messages):
    """
    messages: list[dict]，即 data["messages"]
    ...
    for msg in messages:
        ...
        if role == "user":
            ...
            history_QA.append([current_question, text])
            current_question = None
...
async def async_request_eb_openai_chat_completions(
    ...
    async with aiohttp.ClientSession(trust_env=True, timeout=AIOHTTP_TIMEOUT) as session:
        ...
        prompt = request_func_input.history_QA[-1].get("content", "")
        s_output = {
            "req_id": str(request_func_input.no),
            "text": prompt,
            "history_QA": messages_to_history_qa(request_func_input.history_QA),
            "eda_result": "<response>",
            "chufa_label_texts": "",
            "search_list": [],
            "max_dec_len": 65536,
            "system": "",
            "tokens_all": output.generated_text,
            "is_truncated": 0,
            "tokens_all_num": output.output_tokens
        }
        with open("output.txt", "a") as f:
            f.write(json.dumps(s_output, ensure_ascii=False) + "\n")
