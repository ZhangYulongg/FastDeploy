...
async def async_request_eb_openai_chat_completions(
    ...
    """Request an LLM using EB OpenAI (非流式请求版本)"""
            ...
            async with session.post(url=api_url, json=payload, headers=headers) as response:
                if response.status == 200:
                    ...
                    if choices := data.get("choices"):
                        output.generated_text = choices[0].get("message", {}).get("content", "")
                        output.reasoning_content = choices[0].get("message", {}).get("reasoning_content", "")
                    ...
                    output.arrival_time.extend([time.perf_counter() for _ in range(3)])
                else:
